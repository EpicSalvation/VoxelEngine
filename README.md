...
- [ ] **Document scope-boundary decisions (sanity-check B1/C1/C2):** explicitly document that **kinematic bodies** (B1), **input abstraction / action mapping** (C1), and **UI/HUD widgets** (C2) are **game-owned, not engine-tier** for 1.0. The engine provides the collision primitive (`VoxelCollision::moveAABB`), raw GLFW input, and bgfx rendering — games and plugins compose higher-level systems from these. Record the decision in ARCHITECTURE.md §18 (or a new "Scope boundaries" section) so it reads as a conscious call, not an omission
