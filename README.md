...
- [ ] **Demo — Shared world:** `11-shared-world` — launch two instances on the same machine (one as host, one as client connecting to `localhost`); both clients share one authoritative world running the `server-authority` plugin. Walk and build in the same region — left/right-click edits appear on both screens within one round-trip, the targeted voxel highlight and `on_voxel_modified` HUD readout fire with the correct source on both sides. Fly toward a composite block to decompose it — each client triggers decomposition independently and gets the identical child grid from the shared seed, with no decomposition data crossing the wire. The chat plugin is loaded: type a message in one window and see it appear in the other. The HUD shows connected player count, per-peer position markers (colored cubes), and live edit-packet rate. Controls + run lines added to the Setup section when the demo lands
