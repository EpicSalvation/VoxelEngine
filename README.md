...
> inherited `seed_parameters` are carried onto the produced child macro voxels so a
> grandchild's later decomposition sees its grandparent's constraints, re-derived
> deterministically so eviction stays transparent. Scope boundaries: streaming
...
- [ ] Design task: pin down (a) the **cascade orchestration home** — lift the approach-trigger/`drain`/insert/evict loop out of the demo main loops into a reusable engine component (proposed `src/world/DecompositionManager.{h,cpp}`) owning a `DecompositionState` **per composite layer** (today's single demo-owned state is replaced by a `layer_name → DecompositionState` map), driving the chain through `World::childLayer`/`DecompositionWorker` so a demo only feeds it the camera and drains nothing by hand; (b) the **eviction policy and memory budget** — the resident-set metric (resident chunk count and/or estimated bytes, per layer and/or global), the eviction trigger (the existing `LODManager` distance + hysteresis as the primary signal, plus a hard budget cap that forces eviction of the farthest/least-recently-used **clean** chunks when exceeded), and the dirty-chunk rule (saved through the M4/M7b `ChunkPersistence`/`WorldSave` path before its in-RAM drop, never silently discarded); (c) the **cross-step seed-parameter representation** — where inherited effective `seed_parameters` live on a produced child macro voxel and how they are re-derived identically on regen (ARCHITECTURE §6 line "handoff seam"); (d) the **transitive determinism + occupancy guarantees** — every level pure in `(world_seed, macro VoxelCoord)`, and the §4 "coarse occupancy must superset fine occupancy" invariant now holding across *every* hop of the chain, not just one. Rewrite ARCHITECTURE §4 (and the §6 hierarchical-constraints seam and §11 cache-eviction section) from aspirational to as-built

*Cascade orchestration — the full N-layer chain*
- [ ] `DecompositionManager` (engine-owned) replacing the per-demo loop: holds a `DecompositionState` per composite layer, and each tick (1) enqueues the undecomposed macro voxels within that layer's approach radius whose parent is already decomposed and resident, (2) drains `DecompositionWorker` results into the child layer's `ChunkStore` and marks them decomposed, and (3) runs the budgeted eviction pass — the same operations demos 05/07/09 hand-roll, now written once
...
- [ ] The transitive **coarse-supersets-fine** invariant (§4) enforced/validated across the whole stack: every solid voxel at a fine layer falls within a solid macro voxel at *every* coarser ancestor, so the deep chain never leaves invisible non-collidable holes where a fine surface crosses a coarse boundary (the layered-world footprint-extreme fix generalized to N levels, checked at config-validate time)

*Cache eviction and memory budget*
- [ ] Configurable per-layer memory budget (resident-chunk and/or estimated-byte cap, sourced from `LayerDef`/`LayerConfig`) enforced by the manager across the deep stack: the distance + hysteresis signal evicts normally, and when a layer's resident set still exceeds its budget the farthest/least-recently-used **clean** chunks are evicted to fit, while near and dirty chunks are pinned. The pass never blocks the main thread (it only releases already-drained, resident chunks)
- [ ] **Cascade eviction:** evicting a parent macro voxel back to atomic evicts every decomposed descendant across *all* deeper layers in one consistent pass (today's demo unwinds exactly one level), tearing down each layer's `DecompositionState` entry and its meshes/chunks with no orphaned resident child grids and no double-free — the inverse of the level-by-level decomposition walk
- [ ] **Clean vs dirty on eviction** (ARCHITECTURE §11): a clean recipe-built descendant chunk is dropped and regenerated deterministically on re-approach (nothing persisted); a dirty (player-edited) descendant is saved through the existing `ChunkPersistence`/`WorldSave` path before its in-RAM drop and reloaded on demand, and the composite decomposition state is persisted so a revisit restores rather than silently re-runs. Immutable layers stay outside the dirty/evict cycle as before
- [ ] Fire the `register_on_chunk_created` / `register_on_chunk_evicted` lifecycle hooks consistently from the engine-owned eviction/decomposition path (today only demo 02 fires them by hand), so a plugin observing chunk lifecycle sees every layer of the cascade through one code path — no `plugin_api.h` change (the hooks already exist)

*Hierarchical seed parameters — the cross-step cascade (M9 handoff seam)*
- [ ] Carry inherited effective `seed_parameters` onto each produced child macro voxel (the §6 line-254 deferral): when a composite layer decomposes, the merged param set that biased *its* generation is attached to the child macro voxels it produces, so when one of those children later decomposes, `resolveRecipe`'s merge (M9, per-entry wins) sees grandparent → parent → child params, not just the immediate recipe — extending the single-step merge to the whole chain without either recipe knowing about the other
- [ ] Determinism of the inherited cascade across eviction: the inherited param set is re-derived from the deterministic ancestor decomposition (or persisted and restored), never lost on a clean evict, so a leaf two or more levels down regenerates byte-for-byte identically after its whole ancestor chain is evicted and re-approached — the property that makes the deep cache transparent

*Tests*
- [ ] Full ≥4-layer cascade (extends the M6 `DecompositionTest` / M9 `RecipeDecomposition` harness): approaching drives each level to decompose in declared order; a terminal voxel deep in the chain is resident only after its entire ancestor chain decomposed; each composite layer's `DecompositionState` bookkeeping (pending/decomposed/clear) stays consistent per layer
- [ ] **Cache-miss determinism across the cascade:** decompose the full chain in a region, evict the whole region back to the top atomic block, re-approach — the regenerated deep grid is byte-for-byte identical to the first descent, run repeatedly and concurrently (the M9 determinism guarantee, now end-to-end through every hop)
- [ ] **Cascade eviction correctness:** evicting a parent evicts every decomposed descendant across all deeper layers with consistent per-layer state and no orphaned resident chunks/meshes; a *dirty* descendant is saved (not dropped) on eviction and reloads identically, while a clean sibling regenerates from the recipe
- [ ] **Memory budget:** under a deep stack a layer's resident set stays within its configured budget; exceeding it forces eviction of the farthest/least-recently-used clean chunks while near and dirty chunks remain pinned; the budget pass introduces no main-thread block and no nondeterminism
- [ ] **Cross-step seed-parameter cascade:** a top-level `seed_parameter` measurably biases a leaf two or more levels down (proving the grandparent constrains the grandchild, property-style, not by id branching), each value individually deterministic and byte-identical across an evict → regen cycle
