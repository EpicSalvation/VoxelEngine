...
*README honesty (T7)*
- [ ] Annotate the *Standard tool interoperability* bullet and the *Voxel Editor Interoperability* table with a **not-yet-implemented** note for the textured (Blockbench) path, pointing at this milestone, so the promise matches reality while the work lands; correct it to the textured reality once T1–T6 ship

*Rendering foundation (T1, T2)*
- [ ] **UVs in the vertex format (T1):** add a `TexCoord0` (and, if the atlas is indexed, a tile id) to `MeshVertex` (`ChunkMeshData.h`) and the matching `VoxelVertex::layout` (`BgfxRenderer.cpp:79-81`), keeping the two byte-compatible (`ChunkMesh.cpp` uploads `MeshVertex` memory through the layout); the per-face color stays as a shade/translucency modulate
- [ ] **Atlas sampling in the shaders (T2):** thread the UV through `shaders/varying.def.sc`, bind a texture-atlas sampler, and have `fs_voxel.sc` sample it and modulate by the vertex color; rebuild and commit bytecode for all five backends (`shaders/generated/{dxbc,essl,glsl,metal,spirv}`, ARCHITECTURE §9). Default to a 1×1 white tile so existing colored worlds render **byte-identically** until content opts in

*Texture-atlas asset pipeline (T3)*
- [ ] **Image ingest + atlas upload (T3):** add a minimal texture-asset path — decode an image (header-only decoder or bgfx's bimg, consistent with existing vendoring), build/load a **texture atlas**, and upload via `bgfx::createTexture*`. Owner-tracked per the §8 registry-teardown contract (atlas/tiles registered by a plugin are torn down on unload), mirroring the M12 audio backend's ownership model

*Material→tile binding and per-face emission (T4, T5)*
- [ ] **`(palette_index, face) → atlas tile` table + entry point (T4):** an engine-side table and a plugin call (e.g. `set_material_faces(ctx, palette_index, top, bottom, side…)`) echoing `set_palette_color`, so a "grass block" maps its top face to a grass tile and its sides to dirt — **no field added to `Voxel`/`MaterialProperties`**; the index already on every voxel is the key, preserving the POD, the memcmp determinism padding, RLE persistence (§9), and the plugin ABI
- [ ] **Per-face UV emission in the mesh builder (T5):** in the `ChunkMeshData.cpp` face-emit loop, look up the material's tile for `kFaces[i]`, compute the four corner UVs into the atlas, and write them into the extended `MeshVertex`; keep `kFaces[i].shade` as a color modulate so lit relief survives

*Blockbench import (T6)*
- [ ] **Blockbench importer plugin (T6):** on the existing `register_importer` seam, a handler that ingests a Blockbench export (native `.bbmodel` JSON — per-face texture refs + UVs + referenced image — or its Minecraft block-model / OBJ+MTL forms): load the texture into the atlas (T3), register materials whose face tiles point at the imported regions (T4), and fill the voxel grid with them. Exporter / round-trip is a follow-on **decision item** — recommend one-way import first

*Tests*
- [ ] `tests/VertexLayoutTest.cpp`: `MeshVertex` and `VoxelVertex::layout` stay byte-compatible with the new UV attribute; a mesh built for an untextured (white-tile) material is byte-identical to the pre-milestone color-only output (regression)
- [ ] `tests/MaterialFaceTableTest.cpp`: `set_material_faces` binds tiles per face and the mesh builder emits the bound tile's UVs for each of the six `kFaces` directions; an unbound material falls back to the white tile; `Voxel`/`MaterialProperties` remain trivially-copyable and memcmp-stable (the determinism padding is untouched)
- [ ] `tests/TextureAtlasTest.cpp`: an image ingests into the atlas, tiles resolve to the correct UV regions, and the atlas + its tiles are torn down when the registering plugin unloads (the §8 teardown contract)
- [ ] `tests/BlockbenchImportTest.cpp`: a sample `.bbmodel` imports to the expected materials + face tiles and a voxel grid; the referenced texture lands in the atlas; an unsupported/malformed model fails per the importer contract rather than producing a silently-wrong block

*Demo*
- [ ] **Demo — Textured blocks:** a hand-authored Blockbench **grass block** (green top, dirt sides) and a second multi-texture block, imported and rendered end-to-end — the milestone's acceptance artifact, with the sample `.bbmodel` + texture committed under `assets/`. Proves the painting → export → import → render workflow the *Standard tool interoperability* bullet promises
