...
- [x] Design task: pinned the fluid/heat model and its split (2026-06-16). (a) **State location** — temperature and fluid-amount are *dynamic* fields that don't belong on `MaterialProperties` (which holds constants), so they live in **engine-owned sparse overlays** keyed by coord, ambient/zero default, only non-ambient cells stored (planetary-scale-friendly; no per-voxel memory cost; the §9 chunk format is untouched). (b) **Engine/plugin split** — the engine owns the **field solver** and writes *only* to its overlays, **never to `World` voxels**, so the §13 "engine never writes voxels" invariant holds by construction; a fluid cell crossing saturation fires a `FluidEvent` and a **mandatory `flow` response plugin** realizes it as a real water/lava voxel via the public edit path, exactly mirroring M13's detect/respond feedback loop (no plugin ⇒ no fluid geometry, the legitimate config). (c) **Algorithm** — explicit **cellular-automaton relaxation**: heat = explicit finite-difference diffusion sub-stepped for stability (3D `α·dt/dx² ≤ 1/6`), fluid = level/head flow gated by `porosity`; both reuse the M13 6-connected neighbor pass at **terminal-voxel granularity bounded to the resident region**, driven by the sparse active-set + its frontier, **no LOD aggregation** up the chain. (d) **Property mapping** — `porosity` gates fluid flow (`0` blocks entirely, `1` fully permeable; **air/empty = effective porosity 1.0** so fluid flows freely through open space and only solid low-`porosity` blocks it); `thermal_conductivity` sets the heat diffusion coefficient (conductivity-only / flat heat capacity for M14 — density-derived capacity is a later refinement); ambient is the sparse overlays' default. (e) **Sources** — **plugin-registered emitters** (`register_heat_source`/`register_fluid_source`, owner-tracked and torn down on unload like the other registries) inject into the overlays; there is no material-baked source (a "lava radiates heat" effect is a plugin registering an emitter, not automatic). (f) **Decoupled** — two independent overlays, no boil/freeze coupling in M14. (g) **Persistence** — durable fluid *is* the real voxels the plugin placed (persist for free via §9); both overlays are **transient**, re-derived on load from re-registered emitters + existing fluid voxels; field net-replication deferred (realized voxels already replicate via the M11 edit path). (h) `plugin_api.h` — append flat-POD `FluidEvent`/`ThermalEvent` payloads + `register_on_fluid_event`/`register_on_thermal_event` and the two source-registration calls (`AudioEvent::Flow`/`Collapse` already reserved). New `tuning::fluid` / `tuning::thermal` knob homes in `src/core/Tuning.h`. Wrote ARCHITECTURE §17 (as-designed), the §8 hook additions, and the §13 dep-map entries

*Engine-owned overlays and the solver — `src/simulation/`*
- [ ] **Sparse field overlays**: an engine-owned per-coord sparse store for temperature and (in-flight) fluid amount, ambient/zero as the absent-cell default, scoped to the resident region. Holds working state only — no voxel data, so the §13 engine-never-writes-voxels invariant is untouched. Persisted state rides on the realized fluid *voxels*, not the overlay (both overlays are transient and re-derived on load)
- [ ] **`src/simulation/ThermalSystem.{h,cpp}`**: explicit finite-difference heat diffusion over the active set + frontier, coefficient from each cell's `thermal_conductivity`, sub-stepped to respect the explicit-scheme stability bound; deterministic sorted-coord iteration, end-of-frame, bounded by `tuning::thermal`. Reads `World` properties, writes only the thermal overlay; fires `on_thermal_event` on threshold crossings
- [ ] **`src/simulation/FluidSystem.{h,cpp}`**: cellular-automaton level/head flow gated by `porosity` (air = 1.0, free flow; low-`porosity` solid blocks), end-of-frame and bounded by `tuning::fluid` with carried overflow; a cell crossing saturation fires `on_fluid_event`. Writes only the fluid overlay — never `setVoxel`; the response plugin realizes geometry. Plugin-registered emitters inject sources each tick
- [ ] **Wire both passes into the frame loop** after edits apply, alongside `DecompositionManager::tick` and `PhysicsSystem::tick`; confirm via the §13 map that neither system gains a `Renderer`/`IO`/`DecompositionWorker` edge

*Plugin API surface — `plugin_api.h` additions (append-only, no existing hook changes)*
- [ ] **POD payloads + hooks**: flat-POD `FluidEvent` (cell coord, level, crossed-threshold) and `ThermalEvent` (cell coord, temperature, crossed-threshold) with `register_on_fluid_event`/`register_on_thermal_event` (typedefs `(const FluidEvent*, void*)` / `(const ThermalEvent*, void*)`), following the M13 `StructuralEvent` ABI rule — no `std::` type crosses the boundary, field order append-only
- [ ] **Source-registration calls**: `register_heat_source` / `register_fluid_source` (owner-tracked in `PluginManager`, torn down on unload like the recipe/material/sound registries); the engine injects from registered emitters into the overlays each tick

*Mandatory response plugin (the engine ships no fluid geometry)*
- [ ] **`flow` example plugin** (`plugins/flow/plugin.cpp`): registers `on_fluid_event` and realizes saturated cells as fluid voxels via the public edit path (`apply_edit`), clearing them as they drain — the reference actuator. Dropping it means fluid simulates as a field but never becomes geometry (the legitimate "no fluid" config, the M13 pattern)

*Tests*
- [ ] `tests/ThermalDiffusionTest.cpp`: heat spreads faster through high-`thermal_conductivity` material than low; the explicit scheme stays stable under the sub-step bound; the field is byte-identical across runs (no `rand`/`time`/unordered iteration); a cell past the event threshold fires `on_thermal_event` exactly once per crossing
- [ ] `tests/FluidFlowTest.cpp`: fluid flows through `porosity == 1` and open air, is **blocked by low-`porosity` solid**, conserves total amount under the budget, carries overflow across frames, and the active set is byte-identical across runs
- [ ] Engine-never-writes-voxel invariant: with the `flow` plugin **unloaded**, `FluidSystem`/`ThermalSystem` advance their overlays and fire events but leave `World` **byte-identical** — proving the detect/respond split holds for fluid exactly as it does for collapse
- [ ] Overlay-is-transient invariant: save/load re-derives the fields from re-registered emitters + existing fluid voxels with no §9 chunk-format change; emitters and event hooks are gone after the owning plugin unloads (the M4 teardown contract)
- [ ] **Demo — Flow and heat:** `14-flow-and-heat` — a plugin-registered fluid emitter releases a fluid that **flows through porous materials and is blocked by low-`porosity` ones** (the `flow` plugin realizing it as voxels), plus a heat emitter whose warmth **spreads at rates set by each material's `thermal_conductivity`**. A HUD line shows active fluid/thermal cell counts and the per-frame event/overflow backlog so the budgeted passes are visible; unloading `flow` leaves the fluid a field-only simulation with no geometry. Run/controls in the demo header and the Setup section
