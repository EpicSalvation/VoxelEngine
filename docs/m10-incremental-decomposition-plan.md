...
### 15. [x] Engine design item: recipe occupancy/carving
...
produce full-cube refinement. **Design delivered:**
`docs/proposals/recipe-occupancy.md` — an optional noise/threshold occupancy
stage on `RecipeDesc` (additive ABI, carve-only, deterministic salting,
resolved on the main thread per §13), with a demo-10 migration path.
Implementation is future work, tracked by the proposal's sketch.
...
| 2026-06-12 | fixes-5 | 13, 14, 15 | P2 polish. (13) `makeJob` no longer re-walks the ancestor chain or re-merges ancestor seed_parameters per macro — cached per layer at construction (`ancestorIdxs`/`ancestorSeedParams`); precedence unchanged. (14) demo render loop frustum-culls chunk meshes with a conservative bounding-sphere test built from the renderer's camera basis/projection (shared `kFarClipM`). (15) design delivered as `docs/proposals/recipe-occupancy.md`: optional noise/threshold occupancy stage on `RecipeDesc` (additive ABI, carve-only, §13 resolution, new seed salt domain), v2 surface-relative boundaries, demo-10 migration path; implementation future work. Full suite green. **All plan items 1–15 now checked off; remaining work is in-demo playtest verification per item.** |
