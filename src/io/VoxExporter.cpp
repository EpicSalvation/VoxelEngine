...
#include "renderer/Palette.h"     // palette::color (engine's live render palette)
...
// Used indices get the engine's live render color (palette::color, the same
// table BgfxRenderer/ChunkMeshData draw with) so the exported file shows exactly
// the colors seen in-engine — and a file imported earlier round-trips back to its
// authored colors (VoxImporter installs them into this palette). Unused get gray.
//
// palette::color returns 0xAABBGGRR (ABGR); the .vox RGBA chunk wants r,g,b,a in
// that byte order. Alpha is preserved so a translucent material (e.g. water)
// round-trips; imported files carry their own alpha (usually 0xff).
std::vector<uint8_t> makeRgbaContent(const bool usedIndices[256]) {
    std::vector<uint8_t> c;
    c.reserve(1024);
    for (int i = 1; i <= 255; ++i) {
        if (usedIndices[i]) {
            const uint32_t abgr = palette::color(static_cast<uint8_t>(i));
            writeU8(c,  abgr        & 0xFF);  // R
            writeU8(c, (abgr >>  8) & 0xFF);  // G
            writeU8(c, (abgr >> 16) & 0xFF);  // B
            writeU8(c, (abgr >> 24) & 0xFF);  // A
        } else {
